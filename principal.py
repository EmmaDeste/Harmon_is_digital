...
glob_po = 0
glob_lbsong = 0
glob_croche_duration = 0.125  # a croche has a musical duration of 125ms
glob_croche_dur_sv = tk.StringVar()
...
def set_croche_duration(duration):
    ...
    print("udration : ", duration, "isdigit: ", duration.isdigit())
    if duration.isdigit():
        dur = float(duration)
        if 0 < dur < 4000:
...
def file_reading(file_name):  #Emma: Une fonction read_line_file(f, num) qui prend en paramètres un nom de fichier et un numéro de ligne et qui retourne le contenu de la ligne en question.
    ...
    while line1 != "":                     # to avoid end of lines and end of files, specific for Mac
